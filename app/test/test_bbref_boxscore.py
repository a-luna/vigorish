...
class TestBBRefBoxscore(BaseTestCase):
    ...
    def test_scrape_bbref_boxscore(self):
        ...
        result = write_bbref_boxscore_to_file(
            boxscore_parsed, folderpath=self.APP_TEST_FOLDER
        )
        self.assertTrue(result.success)
        ...
        game_id_dict = boxscore.game_id_dict
        ...
        self.assertEqual(
            boxscore.game_meta_info.first_pitch_wind, "Wind 16mph from Left to Right"
        )
        self.assertEqual(boxscore.game_meta_info.first_pitch_clouds, "Cloudy")
        self.assertEqual(
            boxscore.game_meta_info.first_pitch_precipitation, "No Precipitation"
        )
        ...
        away_team_total_pitches = sum(
            pitch_stats.pitch_count for pitch_stats in away_team_pitch_stats
        )
        home_team_total_pitches = sum(
            pitch_stats.pitch_count for pitch_stats in home_team_pitch_stats
        )
        total_pitches = away_team_total_pitches + home_team_total_pitches
        total_pitch_appearances = len(away_team_pitch_stats) + len(
            home_team_pitch_stats
        )
