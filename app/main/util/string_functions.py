...
def parse_total_minutes_from_duration(duration):
    errors = []
    e = '\nError: game_duration string was not in expected format: {} (expected: h:mm)'.format(duration)
...
def validate_bbref_game_id(input_str):
    if len(input_str) != 12:
        error =(f'String is not a valid bbref game id: {input_str}\n'
...
def validate_bb_game_id(input_str):
    if len(input_str) != 30:
        error =(f'String is not a valid brooks baseball game id: {input_str}\n'
