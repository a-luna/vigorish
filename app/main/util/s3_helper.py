...
def get_all_pitchfx_logs_for_game_from_s3(session, bbref_game_id):
    ...
    fetch_tasks = [get_brooks_pitchfx_log_from_s3(pitch_app_id, delete_file=False) for pitch_app_id in pitch_app_ids]
    ...
    if task_failed:
        ...
        error = f"{len(s3_errors)} errors occurred attempting to retrieve {len(pitch_app_ids)} pitchfx logs for game {bbref_game_id}:\n{s3_errors}"
        return Result.Fail(error)
