...
T_BROOKS_PITCHLOGSFORGAME_FOLDER = '${year}/brooks_pitch_logs'
...
T_BROOKS_PITCHLOGSFORGAME_KEY = '${year}/brooks_pitch_logs/${filename}'
...
def upload_brooks_pitch_logs_for_game(pitch_logs_for_game, scrape_date):
    ...
    if result.failure:
        return result
    filepath = result.value
    s3_key = Template(T_BROOKS_PITCHLOGSFORGAME_KEY)\
        .substitute(year=scrape_date.year, filename=filepath.name)
...
def get_all_brooks_pitch_logs_scraped(year):
    ...
    scraped_keys = [obj.key
        for obj
        in bucket.objects.all()
        if s3_brooks_pitch_logs_folder in obj.key]
