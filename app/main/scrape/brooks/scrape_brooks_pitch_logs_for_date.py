...
def __parse_pitcher_details(response, game, pitcher_id):
    ...
    result = __parse_team_ids(game, selected_pitcher)
    if result.failure:
        return result
...
def __parse_team_ids(game, selected_pitcher):
    ...
    if team_id.upper() == game.away_team_id_bb:
        id_dict = dict(
            team_id=game.away_team_id_bb,
            opponent_id=game.home_team_id_bb
        )
    else:
        id_dict = dict(
            team_id=game.home_team_id_bb,
            opponent_id=game.away_team_id_bb
        )
    return Result.Ok(id_dict)

def __parse_pitch_counts(response):
