...
def event_is_player_substitution_or_misc(event):
    return (
        "BBRefInGameSubstitution" in str(type(event))
...
def get_all_pfx_data_for_game(session, bbref_game_id):
    ...
    for at_bat_id in all_at_bat_ids:
        pfx_for_at_bat = [
            pfx for pfx in all_pfx_data_for_game
        ...
        for instance_number, pfx_ab_id in enumerate(sorted(pfx_ab_ids_for_at_bat)):
            pfx_for_separate_at_bat = [
                pfx for pfx in pfx_for_at_bat
                if pfx.ab_id == pfx_ab_id
            ]
            for pfx in pfx_for_separate_at_bat:
                pfx.at_bat_id = get_at_bat_id_for_pfx_data(pfx, instance_number)
    all_pfx_data_for_game = []
    for pitchfx_log in pitchfx_logs_for_game:
        pitchfx_log.at_bat_ids = list(set([
            pfx.at_bat_id for pfx in pitchfx_log.pitchfx_log
        ]))
        all_pfx_data_for_game.extend(pitchfx_log.pitchfx_log)
...
def reconcile_at_bat_ids(grouped_event_dict, all_pfx_data_for_game):
    at_bat_ids_from_boxscore = list(set([
        at_bat_id for at_bat_id in grouped_event_dict.keys()
    ]))
    at_bat_ids_from_pfx = list(set([
        pfx.at_bat_id for pfx in all_pfx_data_for_game
    ]))
...
def update_inning_with_combined_data(inning, game_events_combined_data, player_team_dict):
    ...
    total_at_bats_pitchfx_complete = len([
        event for event in inning_events
        if event["pitchfx_data_complete"]
    ])
    ...
    pitch_count_pitchfx = sum(
        event["pitch_count_pitchfx"] for event in inning_events
    )
    ...
    at_bat_ids_missing_pitchfx = sorted(list(set(
        event["at_bat_id"] for event in inning_events
...
def audit_pitchfx_vs_bbref_data(
    ...
    total_batters_faced_bbref_home = sum(
        pitch_stats["batters_faced_bbref"]
