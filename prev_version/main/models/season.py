...
class Season(Base):
    ...
    def total_days_scraped_bbref(self):
        return sum(
            date_status.scraped_daily_dash_bbref
    ...
    def percent_complete_bbref_games_for_date(self):
        return (
            self.total_days_scraped_bbref / float(self.total_days)
    ...
    def total_days_scraped_brooks(self):
        return sum(
            date_status.scraped_daily_dash_brooks
            for date_status in self.scrape_status_dates
        )
    ...
    def percent_complete_brooks_games_for_date(self):
        return (
            self.total_days_scraped_brooks / float(self.total_days)
            if self.total_days > 0
            else 0.0
        )
    ...
    def total_bbref_boxscores_scraped(self):
        return sum(
            game_status.scraped_bbref_boxscore
            for game_status in self.scrape_status_games
        )
    ...
    def total_brooks_pitch_logs_scraped(self):
        return sum(
            game_status.scraped_brooks_pitch_logs
            for game_status in self.scrape_status_games
        )
    ...
    def pitch_appearance_count_bbref(self):
        return sum(
            game_status.pitch_app_count_bbref for game_status in self.scrape_status_games
        )

    @hybrid_property
    def pitch_appearance_count_brooks(self):
        return sum(
            game_status.pitch_app_count_brooks
            for game_status in self.scrape_status_games
        )
    ...
    def total_pitch_count_bbref(self):
        return sum(
            game_status.total_pitch_count_bbref
            for game_status in self.scrape_status_games
        )
    ...
    def total_pitch_count_brooks(self):
        return sum(
            game_status.total_pitch_count_brooks
            for game_status in self.scrape_status_games
        )
    ...
    def is_date_in_season(cls, session, check_date, season_type="Regular Season"):
        ...
        if not season:
            error = (
                f"Database does not contain info for MLB {season_type} {check_date.year}"
            )
    ...
    def all_regular_seasons(cls, session):
        matching_seasons = session.query(cls).filter_by(
