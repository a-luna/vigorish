...
from app.main.status.update_status_brooks_pitch_logs import (
...
class ScrapeBrooksDailyPitchLogs(BaseTask):
    ...
    def execute(self, scrape_date):
        ...
        result = scrape_brooks_pitch_logs_for_date(
            brooks_pitch_logs_for_date, brooks_games_for_date
        )
        if result.failure:
            return result
        ...
        with tqdm(
            ...
            for scraped_pitch_logs in scraped_games:
                pbar.set_description(
                    self.get_pbar_upload_description(scraped_pitch_logs.bbref_game_id)
                )
                ...
                pbar.update()
        with tqdm(
            total=len(scraped_games), unit="game", leave=False, position=2
        ) as pbar:
            for scraped_pitch_logs in scraped_games:
                pbar.set_description(
                    self.get_pbar_updating_description(scraped_pitch_logs.bbref_game_id)
                )
                result = update_status_brooks_pitch_logs_for_game(
                    self.db["session"], scraped_pitch_logs, scrape_date
                )
                if result.failure:
                    return result
                pbar.update()
