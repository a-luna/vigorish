...
def update_data_set_brooks_pitch_logs(session, season):
    ...
    result = update_status_brooks_pitch_logs_for_game_list(
        session, season, new_brooks_game_ids
    )
    if result.failure:
        return result
    return Result.Ok()
...
def update_status_brooks_pitch_logs_for_game_list(session, season, new_brooks_game_ids):
    for brooks_game_id in new_brooks_game_ids:
        ...
        if result.failure:
            return result
        result = create_pitch_appearance_status_records(
            session, season, pitch_logs_for_game
        )
        if result.failure:
            return result
    return Result.Ok()
...
def update_game_status_records(session, pitch_logs_for_game):
    try:
        ...
        setattr(
            ...
            pitch_logs_for_game.total_pitch_count,
        )
        return Result.Ok()
...
def create_pitch_appearance_status_records(session, season, pitch_logs_for_game):
    ...
    scraped_pitch_app_ids = [
        plog.pitch_app_id for plog in pitch_logs_for_game.pitch_logs
    ]
    ...
    for pitch_log in pitch_logs_for_game.pitch_logs:
        ...
        try:
            ...
            game_status = GameScrapeStatus.find_by_bb_game_id(
                session, pitch_log.bb_game_id
            )
