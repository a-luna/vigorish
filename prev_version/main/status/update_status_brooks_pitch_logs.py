...
def update_game_status_records(session, pitch_logs_for_game):
    try:
        ...
        game_status = GameScrapeStatus.find_by_bb_game_id(session, brooks_game_id)
        if not game_status:
            error = f"scrape_status_game does not contain an entry for brooks_game_id: {brooks_game_id}"
...
def create_pitch_appearance_status_records(session, season, pitch_logs_for_game):
    ...
    for pitch_log in pitch_logs_for_game.pitch_logs:
        ...
        try:
            ...
            game_status = GameScrapeStatus.find_by_bb_game_id(session, pitch_log.brooks_game_id)
            ...
            setattr(pitch_app_status, "brooks_game_id", pitch_log.brooks_game_id)
