        ...
        for ab_id in ab_ids:
            ab_result = set(
                [
                    pfx.des
                    for pfx in self.pitchfx_log
                    if pfx.ab_id == ab_id and pfx.des in AT_BAT_RESULTS_HIT
                ]
            )
            if ab_result:
                hits += 1
        ...
        for ab_id in ab_ids:
            ab_result = set(
                [
                    pfx.des
                    for pfx in self.pitchfx_log
                    if pfx.ab_id == ab_id and pfx.des in AT_BAT_RESULTS_K
                ]
            )
            if ab_result:
                ks += 1
        ...
        for ab_id in ab_ids:
            ab_result = set(
                [
                    pfx.des
                    for pfx in self.pitchfx_log
                    if pfx.ab_id == ab_id and pfx.des in AT_BAT_RESULTS_BB
                ]
            )
            if ab_result:
                bbs += 1
        ...
        for ab_id in ab_ids:
            ab_result = set(
                [
                    pfx.des
                    for pfx in self.pitchfx_log
                    if pfx.ab_id == ab_id and pfx.des in AT_BAT_RESULTS_HBP
                ]
            )
            if ab_result:
                hbps += 1
