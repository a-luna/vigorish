...
class BBRefBoxscore:
    ...
    def pitch_appearance_count(self):
        return (
            self.away_team_pitch_appearance_count + self.home_team_pitch_appearance_count
        )

    @property
    def away_team_pitch_count(self):
        return sum(
            int(pitch_stats.pitch_count)
            for pitch_stats in self.away_team_data.pitching_stats
        )
    ...
    def home_team_pitch_count(self):
        return sum(
            int(pitch_stats.pitch_count)
            for pitch_stats in self.home_team_data.pitching_stats
        )
