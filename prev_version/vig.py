...
from app.main.setup.populate_tables import populate_tables
...
def setup(db):
    ...
    result = populate_tables(db["session"])
    if result.failure:
...
def status_date(db, game_date, missing_ids, with_games):
    ...
    click.secho(
        f"\n### STATUS REPORT FOR {game_date.strftime(MONTH_NAME_SHORT)} ###", fg="cyan", bold=True
    )
...
def status_season(db, year, verbosity):
    ...
    if verbosity <= 0:
        ...
        return scrape_status_date_range(
            db, season.start_date, season.end_date, False, False, False
        )
    elif verbosity > 2:
...
def reset(db, game_date, scrape, update):
    ...
    if not scrape:
        ...
        return exit_app_success(
            db, f"Successfully reset database and removed all s3 objects for {game_date_str}"
        )
