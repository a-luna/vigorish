...
class TestBrooksPitchFxLog(BaseTestCase):
    ...
    def test_scrape_brooks_pitchfx_log(self):
        ...
        result = read_brooks_pitch_logs_for_game_from_file(
            self.GAME_ID, folderpath=self.TEST_FILES_FOLDER
        )
        ...
        pitch_log = list(
            set(
                [
                    plog
                    for plog in pitch_logs_for_game.pitch_logs
                    if plog.pitch_app_id == self.PITCH_APP_ID
                ]
            )
        )[0]
        ...
        result = write_brooks_pitchfx_log_to_file(
            pitchfx_log, folderpath=self.APP_TEST_FOLDER
        )
        self.assertTrue(result.success)
        filepath = result.value
        self.assertEqual(filepath.name, f"{self.PITCH_APP_ID}.json")

        result = read_brooks_pitchfx_log_from_file(
            self.PITCH_APP_ID, folderpath=self.APP_TEST_FOLDER
        )
        self.assertTrue(result.success)
