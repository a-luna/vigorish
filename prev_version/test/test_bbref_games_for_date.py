...
class TestBBRefGamesForDate(BaseTestCase):
    ...
    def test_bbref_games_req_sel(self):
        ...
        html_path_req = (
            self.APP_TEST_FOLDER / "bbref_games_req_sel" / "2019-06-13_req.html"
        )
        html_path_sel = (
            self.APP_TEST_FOLDER / "bbref_games_req_sel" / "2019-06-13_sel.html"
        )
        game_date = datetime(2019, 6, 13)
        game_date_str = self.get_game_date_str(game_date)
        bbref_url = self.get_bbref_url_for_date(game_date)
        expected_game_count = 11
        games_for_date_req = self.scrape_games_for_date_from_html(
            game_date, html_path_req
        )
        games_for_date_sel = self.scrape_games_for_date_from_html(
            game_date, html_path_sel
        )
    ...
    def test_bbref_nightmare(self):
        ...
        for gd, game_count in game_dates:
            html_path = (
                self.APP_TEST_FOLDER
                / "bbref_nightmare"
                / f"{gd.strftime('%Y%m%d')}.html"
            )
