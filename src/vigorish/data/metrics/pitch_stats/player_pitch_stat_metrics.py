...
class PlayerPitchStatsMetrics:
    ...
    def by_role(self) -> Dict[str, PitchStatsMetrics]:
        ...
        return {
            "all": PitchStatsMetrics(
                pitch_stats=deepcopy(self.pitch_stats),
                player_id_mlb=self.mlb_id,
                player_id_bbref=self.player_id_bbref,
            ),
            "as_sp": PitchStatsMetrics(
                pitch_stats=deepcopy(pitch_stats_as_sp),
                player_id_mlb=self.mlb_id,
                player_id_bbref=self.player_id_bbref,
                role="SP",
            ),
            "as_rp": PitchStatsMetrics(
                pitch_stats=deepcopy(pitch_stats_as_rp),
                player_id_mlb=self.mlb_id,
                player_id_bbref=self.player_id_bbref,
                role="RP",
            ),
    ...
    def by_team_by_year(self) -> List[PitchStatsMetrics]:
        ...
        for year in sorted(all_seasons):
            ...
            stints_for_year = [
                self._get_pitch_stat_metrics_for_team_for_season(
    ...
    def by_opponent(self) -> Dict[str, PitchStatsMetrics]:
        ...
        return {
            ...
            for team_id_bbref in sorted(all_opponents)
        }
    ...
    def by_opponent_by_year(self) -> Dict[int, Dict[str, PitchStatsMetrics]]:
        ...
        for year, season_id in sorted(all_seasons, key=lambda x: x[0]):
            ...
            by_opponent_by_year[year] = {
                team_id_bbref: self._get_pitch_stat_metrics_vs_team_for_season(team_id, team_id_bbref, year)
                for team_id_bbref, team_id in sorted(all_opponents_for_season, key=lambda x: x[0])
            }
        return by_opponent_by_year

    def _get_pitch_stat_metrics_for_season(self, season_id: int, year: int) -> PitchStatsMetrics:
        pitch_stats_for_season = deepcopy(list(filter(lambda x: x.season_id == season_id, self.pitch_stats)))
        return PitchStatsMetrics(
            pitch_stats=pitch_stats_for_season,
            year=year,
            player_id_mlb=self.mlb_id,
            player_id_bbref=self.player_id_bbref,
        )

    def _get_pitch_stat_metrics_for_team(self, team_id_bbref: str) -> PitchStatsMetrics:
        pitch_stats_for_team = deepcopy(
            list(filter(lambda x: x.player_team_id_bbref == team_id_bbref, self.pitch_stats))
        )
        return PitchStatsMetrics(
            pitch_stats=pitch_stats_for_team,
            player_id_mlb=self.mlb_id,
            player_id_bbref=self.player_id_bbref,
            player_team_id_bbref=team_id_bbref,
        )

    def _get_pitch_stat_metrics_for_team_for_season(
        self, team_id: int, team_id_bbref: str, year: int, stint_number: int
    ) -> PitchStatsMetrics:
        pitch_stats_for_team = deepcopy(list(filter(lambda x: x.player_team_id == team_id, self.pitch_stats)))
        return PitchStatsMetrics(
            pitch_stats=pitch_stats_for_team,
            year=year,
            player_id_mlb=self.mlb_id,
    ...
    def _get_pitch_stat_metrics_vs_team(self, team_id_bbref: str):
        pitch_stats_vs_team = deepcopy(
            list(filter(lambda x: x.opponent_team_id_bbref == team_id_bbref, self.pitch_stats))
        )
        return PitchStatsMetrics(
            pitch_stats=pitch_stats_vs_team,
            player_id_mlb=self.mlb_id,
            player_id_bbref=self.player_id_bbref,
            opponent_team_id_bbref=team_id_bbref,
        )

    def _get_pitch_stat_metrics_vs_team_for_season(
        self, team_id: int, team_id_bbref: str, year: int
    ) -> PitchStatsMetrics:
        pitch_stats_vs_team = deepcopy(list(filter(lambda x: x.opponent_team_id == team_id, self.pitch_stats)))
        return PitchStatsMetrics(
            pitch_stats=pitch_stats_vs_team,
            year=year,
            player_id_mlb=self.mlb_id,
