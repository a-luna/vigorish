...
class PitchApp_PitchFx_All_View(db.Base):
    __table__ = create_view(
        name="pitchapp_pitchfx_all",
        selectable=select(
            [
    ...
    def get_pitchfx_metrics_for_pitch_app(cls, db_engine, pitch_app_id):
        results = db_engine.execute(select([cls]).where(cls.id == pitch_app_id)).fetchall()
        return [dict(row) for row in results][0]


class PitchApp_PitchType_All_View(db.Base):
    __table__ = create_view(
        name="pitchapp_pitchtype_all",
        selectable=select(
            [
                db.PitchFx.pitch_app_db_id.label("id"),
                db.PitchFx.pitcher_id.label("pitcher_id"),
...
class PitchApp_PitchFx_Right_View(db.Base):
    __table__ = create_view(
        name="pitchapp_pitchfx_right",
        selectable=select(
            [
                db.PitchFx.pitch_app_db_id.label("id"),
                db.PitchFx.pitcher_id.label("pitcher_id"),
...
class PitchApp_PitchFx_Left_View(db.Base):
    __table__ = create_view(
        name="pitchapp_pitchfx_left",
        selectable=select(
            [
                db.PitchFx.pitch_app_db_id.label("id"),
                db.PitchFx.pitcher_id.label("pitcher_id"),
